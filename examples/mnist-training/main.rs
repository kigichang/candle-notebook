// Copy from Candle  mnist-training example
// Pytorch 很多的 example 都會用 Sequential 功能，是為了方便開發者去堆疊多種神經層設計的。
// Candle 也有類似的功能 Sequential，但礙於 Rust 不是 script language，
// 無法做到像 Pytorch 這樣方便。其實只要將每個神經層依序呼叫就可以達到 Sequential 的效果。
// 為了 demo Sequential 功能，修改原官方的 mnist-training example。
// 並可以互相比對 Pytorch 與 Candle 不一樣的地方。
